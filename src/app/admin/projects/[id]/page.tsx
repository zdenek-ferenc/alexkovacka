...
// Importujeme novou mazací akci
import { deleteGalleryImage } from "./actions";

type PageProps = { params: { id: string } };

// Definujeme si typ pro fotku, aby byl kód čistší
type Photo = {
  id: number;
  image_url: string;
};
...
export default async function ProjectDetailPage({ params }: PageProps) {
  ...
  // Načteme ID i URL, ID budeme potřebovat pro klíče (key)
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        ...
        <div className="p-8 space-y-8">
          {/* SEKCE 1: HLAVNÍ FOTKA (zůstává stejná) */}
          <div className="p-6 bg-white rounded-lg shadow-md">
            <h2 className="text-lg font-bold mb-4">Hlavní fotka</h2>
            {project.main_image_url && (
              <div className="mb-4"><img src={project.main_image_url} alt="Hlavní fotka" className="h-40 rounded-md object-cover" /></div>
            )}
            <ImageUploadForm projectId={project.id} isMain={true} />
          </div>

          {/* SEKCE 2: GALERIE (VYLEPŠENÁ) */}
          <div className="p-6 bg-white rounded-lg shadow-md">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-4">
                    {(galleryPhotos as Photo[]).map((photo) => (
                        <div key={photo.id} className="relative group">
                            ...
                            {/* Formulář s tlačítkem pro smazání */}
                            <form action={async () => {
                                'use server';
